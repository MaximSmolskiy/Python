...
def int_to_base(number: int, base: int) -> str:
    """
    Convert a given positive decimal integer `number` to base `base`.
...
def sum_of_digits(num: int, base: int) -> str:
    """
    Calculate the sum of digit values in a positive integer `num`
...
def harshad_numbers_in_base(limit: int, base: int) -> list[str]:
    """
    Finds all Harshad numbers smaller than `limit` in base `base`.
    Where `base` ranges from ``2`` to ``36``.
...
def is_harshad_number_in_base(num: int, base: int) -> bool:
    """
    Determines whether `num` in base `base` is a harshad number.
    Where `base` ranges from ``2`` to ``36``.
